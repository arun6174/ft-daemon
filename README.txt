                        ===============================
                        ft-daemon: File transfer daemon
                        ===============================

                 Author: Arunashis Ghose <aghose@i2r.a-star.edu.sg>
...
'app_mode' to 'client'. Set 'filename_ext' to appropriate filename extension(s). Update both
'client_directory_root' and 'server_directory_root'. Make user the value of 'server_directory_root'
matches the value you put for server side ft-daemon application. Put the same password
for 'server_passwd' you put in server side config.
...
ft-daemon attempts to move file(s) from that directory whose last modified time is more than 1 min.
So, please take note on that. If the file(s) are locked by the application ft-daemon will simply
fail to move them to 'in-sync' sub-directory and the file(s) will not be sent to server until the
lock is released.
